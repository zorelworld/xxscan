...
def func(ipaddr):
	...
	sys.stdout.write('bing.com => %s\r' % ipaddr)
	sys.stdout.flush()
	#time.sleep(0.5)
...
def _domain_to_ipblock(domain):
	try:
		ip=socket.gethostbyname(domain)
		ipblock='.'.join(ip.split('.')[:-1])+'.0/24'
	except:
		return sys.exit(0)
	return ipblock

def ip2domain_start(domain):
	ip_blocks=_domain_to_ipblock(domain)
	# 只接受/24结尾的IP段，其它抛弃
	...
	for res in result:
		...
		for ipaddr in domains['bing'].keys():
			reverse_info[ipaddr].extend(domains['bing'][ipaddr].values())
...
if __name__ == "__main__":
	...
	if len(sys.argv) == 2:
		...
		f=open(sys.argv[2],'a')
		reverse_info=ip2domain_start(sys.argv[1])
		for i in reverse_info.values():
			for j in i:
				f.write(j+'\n')
		f.close
	else:
